...
export const api = {
  ...
  async bulkSavePdfCandidates(
    ...
    const itemsToSave: QuestionBankItem[] = selectedCandidates.map(c => ({
      ...
      startPage: Math.min(c.promptStart.pageNumber, c.answerStart.pageNumber),
      endPage: Math.max(c.answerStart.pageNumber, c.answerEnd.pageNumber),
      startJuz: c.startJuz || c.detectedJuz || (c.detectedCategory ? parseInt(c.detectedCategory.replace(/\D/g, '') || '30', 10) : 30),
      endJuz: c.endJuz || c.detectedJuz || (c.detectedCategory ? parseInt(c.detectedCategory.replace(/\D/g, '') || '30', 10) : 30),
      answerMode: 'end_ayah',
