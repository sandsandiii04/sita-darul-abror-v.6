...
-- MIGRATION: TAHAP 7D — REKAP FINAL, MONITORING & EXPORT
-- File: supabase_final_recap_monitoring_migration.sql
--
-- STATUS: PRE-LIVE ONLY.
-- DO NOT EXECUTE TO LIVE DATABASE WITHOUT EXPLICIT AUTHORIZATION.
--
-- PRINSIP ARSITEKTURAL:
-- 1. READ / AGGREGATION / REPORTING LAYER ONLY.
-- 2. TIDAK mengubah atau menghitung ulang nilai yang sudah disubmit.
-- 3. TIDAK membuat tabel nilai duplikat (single authoritative source).
-- 4. Pemisahan nilai original dan remedial selalu traceable.
-- 5. Tanpa MAX(original, remedial) dan tanpa capping KKM.
-- 6. Missing score = NULL (tidak pernah menjadi 0).
-- 7. Hardened SECURITY DEFINER (search_path = public, pg_temp, crypt, RBAC).
-- ============================================================

-- ------------------------------------------------------------
-- RPC 1: get_semester_final_recap (Admin Only)
-- Rekap Otoritatif Semester Final, Nilai Efektif & Monitoring
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS get_semester_final_recap(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
...
-- ------------------------------------------------------------
-- RPC 2: get_semester_monitoring_stats (Admin Only)
-- Ringkasan Statistik Monitoring Operasional
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS get_semester_monitoring_stats(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);

CREATE OR REPLACE FUNCTION get_semester_monitoring_stats(
    p_username TEXT,
    p_password TEXT,
    p_academic_term_id TEXT DEFAULT NULL,
    p_config_id TEXT DEFAULT NULL,
...
DECLARE
    v_res JSON;
BEGIN
    -- Reuses authoritative get_semester_final_recap logic ensuring 100% synchronization
    SELECT get_semester_final_recap(
        p_username, p_password, p_academic_term_id, p_config_id,
        p_uts_period_id, p_uas_period_id, p_class, NULL, NULL
    ) INTO v_res;
    ...
    RETURN json_build_object(
        'success', true,
        'config', v_res->'config',
        'summary', v_res->'summary'
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public, pg_temp;

REVOKE EXECUTE ON FUNCTION get_semester_monitoring_stats(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_semester_monitoring_stats(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) TO anon, authenticated, service_role;


-- ------------------------------------------------------------
-- RPC 3: get_student_evaluation_history (Admin Only)
-- Detail Komprehensif Evaluasi Santri & Timeline Kronologis Otoritatif
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS get_student_evaluation_history(TEXT, TEXT, TEXT, TEXT, TEXT);
