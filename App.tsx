...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Magic links untuk persetujuan absensi
    if ((action === 'approve' || action === 'reject') && id) {
      if (!user) {
        // User belum login, tunggu proses login
        return;
      }
      if (user.role !== 'admin') {
      ...
      if (!hasFetched) {
        // Tunggu data selesai dimuat dari Supabase
        return;
      }
    ...
    if ((action === 'approveRequest' || action === 'rejectRequest') && id) {
      if (!user) {
        // User belum login, tunggu proses login
        return;
      }
