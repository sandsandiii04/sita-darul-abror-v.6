...
interface UTSRecapViewProps {
  user: User;
}

export const UTSRecapView: React.FC<UTSRecapViewProps> = ({ user }) => {
  ...
  const loadStudents = async (periodId: string) => {
    ...
    try {
      ...
      if (res.success && res.students) {
        ...
        setErrorMsg(res.message || 'Gagal memuat rekap nilai UTS.');
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'Terjadi kesalahan sistem saat memuat rekap nilai UTS.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenReview = async (student: ExaminerStudentItem) => {
    ...
    try {
      ...
      if (res.success && res.questions && res.assessments) {
        ...
        alert(res.message || 'Gagal memuat rincian penilaian ujian.');
      }
    } catch (err: any) {
      alert(err.message || 'Gagal memuat rincian ujian.');
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `Rekap_Nilai_UTS_${Date.now()}.csv`);
  ...
  const handleExportExcel = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `Rekap_Nilai_UTS_${Date.now()}.xls`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-center text-emerald-700 shadow-sm shrink-0">
              <Award size={22} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-base sm:text-lg font-black text-slate-800 tracking-tight">
                  Rekapitulasi Nilai UTS Tahfiz
                </h1>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {isAdmin 
                  ? 'Rekapitulasi resmi nilai ujian UTS seluruh santri, status kelulusan KKM, dan ekspor data.'
                  : `Rekapitulasi nilai ujian UTS santri binaan Ustadz ${user.name}.`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap shrink-0">
            ...
            <select
              ...
              className="text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
            >
              {periods.map(p => (
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold transition-colors flex items-center gap-1 shadow-sm"
                title="Ekspor Excel"
              >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 pt-2 border-t border-slate-100">
          ...
          <div className="p-3 rounded-xl bg-emerald-50/70 border border-emerald-200/70">
            <span className="text-[10px] uppercase font-bold text-emerald-700 block">Lulus UTS</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-3 sm:p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
          />
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 flex-wrap">
          ...
          >
            ...
            <option value="PASSED">🟢 Lulus UTS (&ge; KKM)</option>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center text-slate-400 space-y-2">
            <RefreshCcw size={24} className="animate-spin mx-auto text-emerald-600" />
            <p className="text-xs">Memuat rekapitulasi nilai UTS...</p>
          </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                <tr>
                  ...
                  <th className="py-3 px-4">Materi Ujian UTS</th>
                      ...
                      <td className="py-3 px-4 text-center">
                        {isSubmitted ? (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-xl bg-slate-100 hover:bg-emerald-50 hover:text-emerald-800 text-slate-700 text-xs font-semibold transition-colors border border-slate-200 hover:border-emerald-200 inline-flex items-center gap-1"
                          >
                            <span>Rincian</span>
