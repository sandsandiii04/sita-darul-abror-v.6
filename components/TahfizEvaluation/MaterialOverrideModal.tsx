...
export const MaterialOverrideModal: React.FC<MaterialOverrideModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason.trim()) {
      setErrorMessage('Wajib mencantumkan alasan penyesuaian materi secara manual.');
      return;
    }
    ...
    onSaved(updatedData, reason.trim());
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">
              Alasan Koreksi Manual <span className="text-rose-500">*</span>
            </label>
            <textarea
              value={reason}
