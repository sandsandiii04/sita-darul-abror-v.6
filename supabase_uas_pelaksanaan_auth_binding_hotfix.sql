-- ============================================================
-- SQL HOTFIX: Tahap 6B Live Hotfix #1 — Authorization + Examiner Binding + Reassignment
-- SITA — Sistem Informasi Tahfidz Al-Qur’an Darul Abror IBS
-- File: supabase_uas_pelaksanaan_auth_binding_hotfix.sql
--
-- Scope:
-- 1. Defect #1: Explicit role whitelist ('admin', 'teacher') across all UAS RPCs
-- 2. Defect #2: exam_attempts.examiner_user_id bound to official assignment (v_assignment.examiner_user_id)
-- 3. Defect #3: Idempotency path caller verification (parent/wali & unassigned teachers rejected)
-- 4. Reassignment Lifecycle: assign_examiner_secure hardened with status guards & synchronized binding
-- 
-- Invariants:
-- - extensions.crypt(p_password, password)
-- - users.password
-- - SECURITY DEFINER
-- - SET search_path = public, pg_temp
-- - No ALTER TABLE / CREATE TABLE / DROP DATA
-- - No dynamic SQL
-- ============================================================
